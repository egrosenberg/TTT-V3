...
FBO::FBO(unsigned int width, unsigned int height, TTTenum fboType, TTTenum texType, unsigned int samples)
{
    ...
    if (texType == TTTenum::TTT_TEXTURE_CUBEMAP)
    {
        TexGenCubemap();
    }
    else if (fboType == TTTenum::TTT_FRAMEBUFFER)
    {
        TexGen();
    }
    else if (fboType == TTTenum::TTT_MULTISAMPLE_FRAMEBUFFER)
    {
        TexGenMultisample();
    }
    else if (fboType == TTTenum::TTT_DEPTH_FRAMEBUFFER)
    {
        TexGenDepthonly();
    }
    else
...
void FBO::TexGenCubemap()
{
    ...
    if (m_Type == TTTenum::TTT_DEPTH_FRAMEBUFFER)
    {
...
void FBO::BindTex()
{
    switch (m_TexType)
    {
        case TTTenum::TTT_TEXTURE_CUBEMAP:
            glBindTexture(GL_TEXTURE_CUBE_MAP, m_Tex);
        default:
            glBindTexture(GL_TEXTURE_2D, m_Tex);
    }
